...
if (isset($_GET['id_systeme'])) {
	...
	if (isset($_GET['installe'])) {
		...
		$req = $bdd->prepare('UPDATE regulateur_ph_plus SET consommation_jour = :consommation_jour WHERE id_systeme = :id_systeme');
		$req->execute(array(
			'consommation_jour' => (float)$_GET['consommation_jour'],
			'id_systeme' => (int)$result['id']
			));
	} elseif (isset($_GET['consommation_semaine'])) {
		$req = $bdd->prepare('UPDATE regulateur_ph_plus SET consommation_semaine = :consommation_semaine WHERE id_systeme = :id_systeme');
		$req->execute(array(
			'consommation_semaine' => (float)$_GET['consommation_semaine'],
			'id_systeme' => (int)$result['id']
			));
	} elseif (isset($_GET['consommation_mois'])) {
		$req = $bdd->prepare('UPDATE regulateur_ph_plus SET consommation_mois = :consommation_mois WHERE id_systeme = :id_systeme');
		$req->execute(array(
